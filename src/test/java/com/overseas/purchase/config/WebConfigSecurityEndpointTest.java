...
class WebConfigSecurityEndpointTest {

    @Test
    void passwordContactResetIsNotExcludedButPaymentNotificationsAre() throws Exception {
        InterceptorRegistry registry = new InterceptorRegistry();
        WebConfig webConfig = new WebConfig(new AuthInterceptor(mock(JwtUtil.class)));

        webConfig.addInterceptors(registry);

        List<String> excludePatterns = excludePatterns(registrations(registry).get(0));
        assertThat(excludePatterns)
                .doesNotContain("/user/reset-password-by-contact")
                .contains("/payment/notify/wechat", "/payment/notify/alipay");
    }
